...
def get_dashboard():
    ...
    latest_analysis = db.analyses.find_one(
        {"user_id": user_id}, sort=[("created_at", -1)]
    )
    total_skills = db.skills.count_documents({"user_id": user_id})

    matched_count = (
        len(latest_analysis.get("matched_skills", [])) if latest_analysis else 0
    )
    partial_count = (
        len(latest_analysis.get("partial_skills", [])) if latest_analysis else 0
    )
    missing_count = (
        len(latest_analysis.get("missing_skills", [])) if latest_analysis else 0
    )
    readiness_score = (
        latest_analysis.get("readiness_score", 0) if latest_analysis else 0
    )
    ...
    if latest_analysis:
        ...
        combined = list(latest_analysis.get("missing_skills", [])) + list(
            latest_analysis.get("partial_skills", [])
        )
    ...
    data = {
        ...
        "current_target_role": (
            latest_analysis.get("job_role_name") if latest_analysis else None
        ),
    }
